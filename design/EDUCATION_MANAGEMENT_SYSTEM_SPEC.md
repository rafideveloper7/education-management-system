# Education Management System (EduOS)
## Complete Functional Requirements & Technical Specification
...
## Complete Functional Requirements (Modules 1 - 36)
...
**Rule:** Public registration does **NOT** create an academic account.
...
### 8. STUDENT MANAGEMENT (360° PROFILE DOSSIER)

Admin can access the complete 360° student profile containing all 12 operational layers in a unified, visual student dossier:

| # | Profile Layer | Scope & Tracked Information | Administrative Control |
| :---: | :--- | :--- | :--- |
| **01** | **Personal Information** | Full legal name, date of birth, age, gender, blood group, emergency contact phone, home address, and verified high-resolution photograph. | Edit profile, update contact, upload new photo. |
| **02** | **Parent / Guardian Information** | Father name, mother name, CNIC / National ID, guardian relationship, emergency contact coordinates, and primary communication phone. | Link/unlink parents, update guardian details. |
| **03** | **Class & Enrollment** | Enrolled academic year, grade level, assigned section, academic track (Science, Commerce, Arts), and assigned class teacher. | Promote student, change class/section. |
| **04** | **Subjects** | Complete list of enrolled core subjects, electives, and corresponding assigned subject teachers. | Assign electives, adjust subject curriculum. |
| **05** | **Attendance Ledger** | Real-time attendance percentage, total class days, present days, unexcused absences, approved medical leaves, and period check-ins. | Audit attendance, authorize corrections. |
| **06** | **Assignments & Tasks** | Active homework list, deadlines, submitted files, upload timestamps, teacher rubric marks, and qualitative remarks. | Monitor homework compliance & submission. |
| **07** | **Exams & Schedules** | Upcoming examination timetable, exam hall number, desk allotment, generated roll number slip, and attendance on exam days. | Issue roll slips, reschedule examinations. |
| **08** | **Academic Results & GPA** | Term-by-term published marks, grading breakdown, aggregate percentages, cumulative GPA, position rank, and official transcripts. | Publish/unpublish results, print report card. |
| **09** | **Fees & Finance** | Assigned fee structure, monthly tuition vouchers, bank challans, due dates, paid amounts, outstanding balances, and receipt logs. | Generate vouchers, record payments, grant waivers. |
| **10** | **Applications & Requests** | Leave requests, character certificate requests, transfer applications, and document requests with live status progression. | Review, approve, reject, or request information. |
| **11** | **Complaints & Grievances** | Filed concerns, behavioral warnings, counselor remarks, disciplinary board notes, and resolution history with confidentiality lock. | Manage sensitive records, resolve issues. |
| **12** | **Document Vault** | Uploaded birth certificate, National ID / B-Form, previous school leaving certificates, immunization charts, and transfer documents. | Verify documents, download signed files. |
...
#### Possible Statuses:
```text
Submitted ➔ Under Review ➔ Need Information ➔ Approved / Rejected / Resolved
```
...
* Maintain a complete history of every application
...
#### Possible Statuses:
```text
Submitted ➔ Under Review ➔ Assigned ➔ Need Information ➔ In Progress ➔ Resolved ➔ Rejected ➔ Closed
```
...
A proper leave system exists for:
...
The system supports academic scheduling without conflicts.
...
The system supports institutional documents where required.
...
Admin controls system-level settings such as:
...
```text
Student ➔ Application / Complaint ➔ Teacher / Admin / Principal
Parent  ➔ Application / Complaint ➔ Teacher / Admin / Principal
Teacher ➔ Application / Complaint ➔ Admin / Principal
```
...
The system includes a complete **ID Card Generation and Management System** for students and teachers.
**ID cards are generated from the information already stored in the system instead of requiring the same information to be entered again manually.**

#### Student ID Card
After a student's admission is successfully approved and the student account is created, the system makes the student's ID card available.
...
ID Card Generated
        ↓
Student Can View / Download / Print ID Card
```
*The student can view, download, and print the ID card from the Student Panel when the required information is completed.*

#### Teacher ID Card
Teachers also have official institute ID cards.
...
*The fields are configurable by Admin.*

---

## FINAL RESPONSIBILITY MODEL

* **Public Website:** Institution's public presence + admissions + public information.
* **Admin:** Runs and controls the institution.
* **Teacher:** Teaches and manages assigned academic responsibilities.
* **Student:** Uses and manages their own learning experience.
* **Parent:** Monitors and communicates about their children.
* **Communication System:** Connects users through controlled applications, requests and complaints.
* **Notification System:** Keeps users informed about important system activities.
* **Dynamic Website Management:** Allows Admin to control the public website's content, branding, theme, structure and institute information without developer intervention.
